...
export default function PortfolioArchive({ onSelectProject }: PortfolioArchiveProps) {
  ...
  // Dynamic filter tags computed from both categories and existing projects
  const filters = useMemo(() => {
    const list = ["ALL WORKS"];
    if (categories && categories.length > 0) {
      categories.forEach(cat => {
        if (cat.name && !list.includes(cat.name.toUpperCase())) {
          list.push(cat.name.toUpperCase());
        }
      });
    } else if (projects && projects.length > 0) {
      projects.forEach(p => {
        const cat = p.category;
        if (cat && !list.includes(cat.toUpperCase())) {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      if (selectedFilter === "ALL WORKS") return true;

      const projectCat = (project.category || "").toUpperCase();
      const projectTags = (project.tags || []).map(t => t.toUpperCase());

      return (
        projectCat === selectedFilter ||
        projectTags.includes(selectedFilter) ||
        projectCat.includes(selectedFilter) ||
        // Fuzzy tag mapping
        (selectedFilter === "POSTERS" && projectTags.includes("CAMPAIGN")) ||
        (selectedFilter === "GRAPHIC DESIGN" && (projectTags.includes("BRANDING") || projectTags.includes("LOGO DESIGN")))
      );
    });
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.length > 0 ? (
              ...
              <div className={`col-span-full py-20 text-center border transition-colors duration-300 ${
                ...
                <button 
                  onClick={() => { setSelectedFilter("ALL WORKS"); }}
